...
const Menu = () => {
    ...
    return (
        <div className='flex justify-between items-center w-full py-5 md:px-0'>
            <div className='hidden md:block md:pl-6 py-2'>
            ...
            <div className="absolute right-4 top-3 md:hidden"> 
               <button onClick={() => setProfile(!profile)} className="focus:outline-none">
                  <img className='w-8 h-8 rounded-full bg-gray-100 p-1 cursor-pointer' src="images/menu-line.png" alt="Menu" /> 
               </button>
            </div>
            ...
            <div className='hidden md:flex text-gray-500 mr-10 lg:mr-20 gap-8 items-center'>
                ...
                <div onClick={() => setProfile(!profile)} className='relative flex gap-2 items-center cursor-pointer hover:bg-gray-50 p-1 rounded'>
                    ...
                    <span className=" cursor-pointer text-xs font-semibold max-w-[100px] truncate">{user?.user?.username || "USER"}</span>
                    ...
                    {profile && (
                        <div className='absolute top-full right-0 mt-2 w-48 bg-white shadow-xl rounded-lg border border-gray-100 z-50 overflow-hidden py-1 animate-in fade-in zoom-in-95'>
                             <div className="px-4 py-3 border-b border-gray-100 bg-gray-50">
                                <p className="text-sm font-medium text-gray-900 truncate">{user?.user?.username}</p>
            ...
            {profile && (
                <div className="md:hidden absolute top-14 left-0 right-0 bg-white border-b border-gray-200 shadow-xl z-50 p-4 animate-in slide-in-from-top-2">
                     <div className="flex items-center gap-3 mb-4 p-3 bg-gray-50 rounded-lg border border-gray-100">
                        <div className="w-10 h-10 cursor-pointer bg-purple-600 text-white rounded-full flex items-center justify-center text-lg font-bold">
                            {user?.user?.username?.[0]?.toUpperCase() || "U"}
                        </div>
                        <div className="overflow-hidden">
                            <p className="font-semibold text-gray-900 truncate">{user?.user?.username}</p>
                            <p className="text-xs text-gray-500 truncate">{user?.user?.email}</p>
                        </div>
                     </div>
                     <ul className="space-y-2">
                        <Link to='/summary' onClick={() => setProfile(false)}><li className="cursor-pointer block p-3 rounded-lg bg-blue-50 text-blue-700 font-medium">Dashboard</li></Link>
                        <Link to='/orders' onClick={() => setProfile(false)}><li className="cursor-pointer block p-3 rounded-lg hover:bg-gray-50 text-gray-600">Orders</li></Link>
                        <Link to='/holdings' onClick={() => setProfile(false)}><li className="cursor-pointer block p-3 rounded-lg hover:bg-gray-50 text-gray-600">Holdings</li></Link>
                        <Link to='/funds' onClick={() => setProfile(false)}><li className="cursor-pointer block p-3 rounded-lg hover:bg-gray-50 text-gray-600">Funds</li></Link>
                        <li onClick={() => { navigation('/support'); setProfile(false); }} className="cursor-pointer block p-3 rounded-lg hover:bg-gray-50 text-gray-600">Support</li>
                        <li onClick={logout} className="cursor-pointer block p-3 rounded-lg text-red-600 font-medium hover:bg-red-50 mt-2 border-t border-gray-100">Logout</li>
                     </ul>
                </div>
            )}
        </div>
